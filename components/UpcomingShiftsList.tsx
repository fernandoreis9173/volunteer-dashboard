...
const ScheduleCard: React.FC<{ schedule: DashboardEvent; onViewDetails: (event: DashboardEvent) => void; userRole: string | null; leaderDepartmentId?: number | null; onMarkAttendance: (event: DashboardEvent) => void; onViewTimeline: (event: DashboardEvent) => void; isToday: boolean; }> = ({ schedule, onViewDetails, userRole, leaderDepartmentId, onMarkAttendance, onViewTimeline, isToday }) => {
  ...
  // FIX: Filter volunteers and departments based on leader's department if applicable
  const relevantVolunteers = useMemo(() => {
    if (!schedule.event_volunteers) return [];
    if ((userRole === 'leader' || userRole === 'lider') && leaderDepartmentId) {
      return schedule.event_volunteers.filter(v => Number(v.department_id) === Number(leaderDepartmentId));
    }
    return schedule.event_volunteers;
  }, [schedule.event_volunteers, userRole, leaderDepartmentId]);

  const relevantDepartments = useMemo(() => {
    if (!schedule.event_departments) return [];
    if ((userRole === 'leader' || userRole === 'lider') && leaderDepartmentId) {
      return schedule.event_departments.filter(d => Number(d.department_id) === Number(leaderDepartmentId));
    }
    return schedule.event_departments;
  }, [schedule.event_departments, userRole, leaderDepartmentId]);

  const volunteerNames = relevantVolunteers.map(v => v.volunteers?.name).filter(Boolean) as string[] ?? [];
  const departmentNames = relevantDepartments.map(d => d.departments?.name).filter(Boolean) as string[] ?? [];
      ...
      <div>
        ...
        <div className="flex justify-between items-end gap-4">
          ...
          <div className="flex items-center space-x-1 flex-shrink-0">
            {isLeader && isToday && hasStarted && !isFinished && (
              <button
...
const FilterButton: React.FC<{ label: string; value: EventFilter; activeValue: EventFilter; onClick: (value: EventFilter) => void }> = ({ label, value, activeValue, onClick }) => (
  <button
    ...
    className={`px-4 py-2 text-sm font-semibold rounded-lg transition-all duration-200 ${activeValue === value ? 'bg-white text-slate-900 shadow-sm' : 'bg-transparent text-slate-500 hover:text-slate-900'
      }`}
  >
    {label}
  </button>
...
const UpcomingShiftsList: React.FC<UpcomingShiftsListProps> = ({ todaySchedules, upcomingSchedules, onViewDetails, userRole, leaderDepartmentId, onMarkAttendance, onViewTimeline }) => {
