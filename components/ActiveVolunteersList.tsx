              ...
              {volunteers.map(v => (
                <div key={v.id} className="flex items-center space-x-4 p-2 rounded-lg hover:bg-slate-50">
                  <div className="flex items-center space-x-3 flex-shrink-0">
                    ...
                    <div className="w-10 h-10 rounded-full bg-blue-500 text-white flex items-center justify-center font-bold text-sm overflow-hidden">
                      {v.avatar_url ? (
                        <img src={v.avatar_url} alt={v.name} className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                      ) : (
                        v.initials
                      )}
                    </div>
