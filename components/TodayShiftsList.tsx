...
const ScheduleCard: React.FC<{ schedule: DashboardEvent; onViewDetails: (event: DashboardEvent) => void; }> = ({ schedule, onViewDetails }) => {
  const { fullDate: formattedDate, time: startTime } = convertUTCToLocal(schedule.date, schedule.start_time);
  const { time: endTime } = convertUTCToLocal(schedule.date, schedule.end_time);
