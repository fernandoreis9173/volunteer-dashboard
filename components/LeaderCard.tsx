...
const LeaderCard: React.FC<LeaderCardProps> = ({ leader, onEdit, onDelete }) => {
  const getAvailabilityText = () => {
    let availabilityData: any = leader.availability;
    ...
    if (typeof availabilityData === 'string' && availabilityData.startsWith('[') && availabilityData.endsWith(']')) {
      try {
        ...
        return String(leader.availability);
      }
    }
  ...
  return (
    <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-200 flex flex-col space-y-4">
      <div className="flex items-start justify-between">
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <div className={`flex items-center space-x-1.5 text-sm font-semibold mt-1 ${leader.status === 'Ativo' ? 'text-green-600' : 'text-slate-500'}`}>
              {leader.status === 'Ativo' ? (
        ...
        <div className="flex items-center space-x-3 text-slate-400">
          <button onClick={() => onEdit(leader)} className="hover:text-slate-600">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
              <path strokeLinecap="round" strokeLinejoin="round" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.5L16.732 3.732z" />
            </svg>
          </button>
          <button onClick={() => onDelete(leader.id!)} className="hover:text-red-600">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
      ...
      <div className="text-sm text-slate-600 space-y-2">
        ...
        {leader.phone && (
          <div className="flex items-center space-x-2">
              ...
              <span>{formatPhoneNumber(leader.phone)}</span>
          </div>
        )}
      </div>
      ...
      {leader.ministries.length > 0 && (
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {leader.ministries.map(m => <Tag key={m} color="yellow">{m}</Tag>)}
          </div>
        </div>
      )}
      ...
      {leader.skills.length > 0 && (
          ...
          <div className="flex flex-wrap gap-2">
              {leader.skills.map(s => <Tag key={s} color="blue">{s}</Tag>)}
          </div>
          </div>
      )}
