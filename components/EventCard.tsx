        ...
        >
            ...
            <div className="absolute top-4 right-4 flex items-center gap-1">
                <button onClick={() => setExpanded(!expanded)} className="p-1.5 text-slate-400 hover:text-slate-600 rounded-md hover:bg-slate-100">
                    <svg xmlns="http://www.w3.org/2000/svg" className={`h-5 w-5 transition-transform ${expanded ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="m19.5 8.25-7.5 7.5-7.5-7.5" /></svg>
                </button>
                {isAdmin && (
                    <>
            ...
            <div className="pr-16 sm:pr-24">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center gap-x-4 gap-y-1 text-sm text-slate-500 mt-1">
                    ...
                    <span className="flex items-center gap-1.5"><svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" /></svg>{event.start_time} - {event.end_time}</span>
                </div>
            </div>
            ...
            <div className="mt-4 flex flex-row justify-start items-center gap-3">
                {isLeader && isToday && isDepartmentInvolved && (
                    <button onClick={() => onMarkAttendance(event)} className="flex-1 md:flex-initial text-center px-4 py-2 text-sm bg-teal-500 text-white font-semibold rounded-lg hover:bg-teal-600 shadow-sm whitespace-nowrap">
                        Marcar Presença
                    </button>
                )}
                {isLeader && canLeaderSchedule && isDepartmentInvolved && (
                     <button onClick={() => onEdit(event)} className="flex-1 md:flex-initial text-center px-4 py-2 text-sm bg-blue-500 text-white font-semibold rounded-lg hover:bg-blue-600 shadow-sm whitespace-nowrap">
                        Escalar Voluntários
                    </button>
                )}
                 {isLeader && canLeaderSchedule && !isDepartmentInvolved && (
                    <button onClick={() => onAddDepartment(event)} className="flex-1 md:flex-initial text-center px-4 py-2 text-sm bg-indigo-500 text-white font-semibold rounded-lg hover:bg-indigo-600 shadow-sm whitespace-nowrap">
                        ...
                        <div key={departments.id} className={`p-4 rounded-lg ${isLeadersDept ? 'bg-blue-50' : 'bg-slate-50/70'}`}>
                            <div className="flex justify-between items-start">
                                ...
                                <span className={`flex items-center space-x-1.5 text-sm font-semibold ${hasScheduled ? 'text-green-600' : 'text-amber-600'}`}>
                                    {hasScheduled ? <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg> : <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>}
