...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: inviteData, error: inviteError } = await supabaseAdmin.auth.admin.inviteUserByEmail(
      email,
      { data: { name: name, status: 'Pendente', role: normalizedRole } }
    );
    ...
    const { data: updatedUserData, error: updateError } = await supabaseAdmin.auth.admin.updateUserById(
      inviteData.user.id,
      {
        user_metadata: { name: name, status: 'Pendente', role: normalizedRole }
      }
    );
    ...
    const { error: profileError } = await supabaseAdmin
      .from('profiles')
      .insert({ id: finalUser.id, role: normalizedRole });
